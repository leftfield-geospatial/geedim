...
@pytest.mark.parametrize(
    ...
    [
        ('sub2/file', 'folder/sub1', 'projects/folder/assets/sub1/sub2/file'),
        ('file', None, 'file'),
    ],
)
...
def test_get_tqdm_kwargs(kwargs: dict):
    ...
    assert (
        tqdm_kwargs['unit'] == kwargs['unit']
        if 'unit' in kwargs
        else 'unit' not in tqdm_kwargs
    )
...
def test_async_runner():
    ...
    # test running a coroutine when another event loop is already running in this
    # thread (e.g. jupyter notebook)
...
def test_auto_leave_tqdm():
    ...
    with utils.auto_leave_tqdm() as outer_bar:
        ...
        with utils.auto_leave_tqdm() as inner_bar:
            assert inner_bar.leave is False
