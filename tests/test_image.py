...
@pytest.mark.parametrize(
    ...
    [
        ('s2_sr_hm_image', 1, True),
        ('l9_sr_image', 0, True),
        ('modis_nbar_image', 0, False),
    ],
)
def test_properties(
    ...
    assert image.date == datetime.fromtimestamp(
        props['system:time_start'] / 1000, tz=UTC
    )
    ...
    exp_size = np.dtype(image.dtype).itemsize * np.prod(image.shape) * image.count
    assert image.size == exp_size
...
def test_dtype(ee_dtypes: list, exp_dtype: str):
    ...
    ee_dtypes = [
        dict(zip(['precision', 'min', 'max'], dt, strict=True)) for dt in ee_dtypes
    ]
...
@pytest.mark.parametrize(
    'patch_export_task', ['export_task_success_sequence'], indirect=True
)
...
def test_monitor_task_error(
    patch_export_task, export_task_fail, capsys: pytest.CaptureFixture
):
...
def test_projection(s2_sr_hm_image: ImageAccessor):
    ...
    projs = ee.List([s2_sr_hm_image.projection(min_scale=ms) for ms in [True, False]])
    projs = projs.getInfo()
...
def test_fixed(
    ...
    landsat_ndvi_image: ImageAccessor,
    const_image: ImageAccessor,
):
    """Test fixed()."""
    # s2_sr_hm_image has a combination of fixed and non-fixed bands,
    # landsat_ndvi_image has one non-fixed band, and const_image has all non-fixed bands
    fixeds = ee.List(
        [s2_sr_hm_image.fixed(), landsat_ndvi_image.fixed(), const_image.fixed()]
    )
...
def test_resample(
    ...
    """Test resample() on fixed and non-fixed projection images with up- and
    downsampling.
    """
    ...
    def get_reproj_std(image: ee.Image, **reproj_kwargs) -> ee.Number:
        ...
        return (
            reproj_im.reduceRegion('stdDev', geometry=region_100ha)
            .values()
            .reduce('mean')
        )

    # find standard deviations before and after resampling for different method and
    # image combinations, combining all getInfo() calls into one
    ...
    for src_im, method, reproj_kwargs in zip(
        ...
        stds = [
            get_reproj_std(im, **reproj_kwargs)
            for im in [src_im._ee_image, resample_im]
        ]
    ...
    # test standard deviations are reduced by resampling when the image has a fixed
    # projection
    for std_test, fixed in zip(std_tests, fixeds, strict=True):
        if fixed:
...
def test_to_dtype(landsat_ndvi_image: ImageAccessor):
    ...
    for converted_image, dtype in zip(converted_images, dtypes, strict=True):
...
def test_scale_offset(
    ...
    min_max_tests = []
    refl_bands_list = []
    for src_im, scale_offset_im in zip(src_ims, scale_offset_ims, strict=True):
    ...
    for min_max_test, refl_bands in zip(
        results['min_max'], refl_bands_list, strict=True
    ):
    ...
    for src_im, scaled_offset_info in zip(src_ims, results['image'], strict=True):
...
def test_region_coverage(prepared_image: ImageAccessor):
    """Test regionCoverage()."""
    # find & test coverages for regions spanning image and mask bounds, combining all
    # getInfo()'s into one (see prepared_image() to understand regions & exp_coverages)
    ...
    coverages = [
        mask.regionCoverage(region=r, scale=prepared_image.scale) for r in regions
    ]
    coverages = ee.List(coverages).getInfo()

    for coverage, exp_coverage in zip(coverages, exp_coverages, strict=True):
...
def test_mask_clouds(l9_sr_image: ImageAccessor, region_100ha: dict):
    ...
    sums = [
        im.mask()
        .reduceRegion('sum', geometry=region_100ha, scale=30)
        .values()
        .reduce('mean')
...
def test_prepare_for_export(s2_sr_hm_image: ImageAccessor, region_100ha: dict):
    ...
    prep_ims = [
        s2_sr_hm_image.prepareForExport(**prep_kwargs)
        for prep_kwargs in prep_kwargs_list
    ]
    prep_ims = accessors_from_images(prep_ims)

    # test prepared image properties
    for prep_im, prep_kwargs in zip(prep_ims, prep_kwargs_list, strict=True):
    ...
    for prep_im in prep_ims[-4:]:
        prep_tform = rio.Affine(*prep_im.transform)
...
def test_prepare_for_export_scale_offset(
    s2_sr_hm_image: ImageAccessor, region_100ha: dict
):
    """Test the prepareForExport() scale_offset parameter."""
    # This just tests the scale_offset parameter was acted on. Detailed scaleOffset()
    # testing is done in test_scale_offset().
    prep_ims = [
...
def test_to_geotiff(
    ...
    with rio.open(file) as ds:
        ...
        # masked pixels will always == _nodata_vals[image.dtype], irrespective of the
        # nodata value
        ...
        for bi in range(image.count):
            band_props = {
                k.replace(':', '-'): str(v) for k, v in image.bandProps[bi].items()
            }
...
@pytest.mark.parametrize(
    'masked, structured', [(False, False), (True, False), (True, True)]
)
def test_to_numpy(
    prepared_image: ImageAccessor,
    prepared_image_array: np.ndarray,
    ...
    if structured:
        ...
        dtype = np.dtype(
            dict(names=image.bandNames, formats=[image.dtype] * image.count)
        )
    ...
    # contents
    array_ = (
        array.view(image.dtype).reshape(*image.shape, image.count)
        if structured
        else array
    )
...
def test_export_error(landsat_ndvi_image: ImageAccessor, tmp_path: Path):
    ...
    # just in case, prevent this test running large exports if landsat_ndvi_image
    # does have a fixed projection
