...
class TestGeeDim(unittest.TestCase):
    ...
    def _test_search(self, imsearch_obj):
        """
        ...
        if self.test_export:    # start export tasks
            export_tasks += download.export_image(image, f'{image_id}_None_None', folder='GeedimTest', region=region, crs=None,
                                      scale=None, wait=False)
            export_tasks += download.export_image(image, f'{image_id}_Epsg32635_240m', folder='GeedimTest', region=region,
                                      crs='EPSG:32635', scale=240, wait=False)
    ...
    def test_geedim(self):
        ...
        for test_obj in test_objs:
            export_tasks += self._test_search(test_obj)
