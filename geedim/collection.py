...
from geedim import export, image, info, composite, root_path
...
class Collection(object):
    def __init__(self, gd_coll_name):
        ...
        self._gd_image_cls =  image.coll_to_im_cls_map[gd_coll_name]
        self._ee_base_coll = self._gd_image_cls.ee_collection()
        self._ee_mapped_coll = None

    @classmethod
    def from_ids(cls, image_ids, mask=False, scale_refl=False):
        ee_coll_name = ee_split(image_ids[0])[0]
        if not ee_coll_name in info.ee_to_gd_map:
            raise ValueError(f'Unsupported collection: {ee_coll_name}')

        id_check = [ee_split(im_id)[0] == ee_coll_name for im_id in image_ids[1:]]
        if not all(id_check):
        ...
        for im_id in image_ids:
            ee_image = gd_collection._gd_image_cls.from_id(im_id, mask=mask, scale_refl=scale_refl)
            im_list = im_list.add(ee_image)

        cls._ee_mapped_coll = ee.ImageCollection(im_list)
        return cls
    ...
    def search(self, start_date, end_date, region, valid_portion=0):
        ...
        def set_valid_portion(ee_image):
            max_scale = export.get_projection(ee_image, min=False).nominalScale()
        ...
        self._ee_mapped_coll = (self._ee_base_coll.
    ...
    def composite(self, method='q_mosaic'):
        ...
        if method == 'q_mosaic':
            ...
            comp_image = composite.medoid(self._ee_mapped_coll, bands=bands)
        else:
...
class ImCollection:
    ...
    def ee_collection_from_ids(self, ids, apply_mask=False, add_aux_bands=False, scale_refl=False):
        """
        ...
        ee_coll_name = ee_split(ids[0])[0]
        if not ee_coll_name == self.ee_coll_name:
            raise ValueError(f'Unsupported collection: {ee_coll_name}')

        id_check = [ee_split(im_id)[0] == ee_coll_name for im_id in ids[1:]]
        if not all(id_check):
            raise ValueError(f'All images must belong to the same collection')

        im_list = ee.List([])
        for im_id in ids:
            im = self.get_image(im_id, apply_mask=apply_mask, add_aux_bands=add_aux_bands, scale_refl=scale_refl)
