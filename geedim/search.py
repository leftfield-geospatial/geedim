...
class ImCollection:
    def __init__(self, collection):
        ...
        self.base_collection = ee.ImageCollection(self.collection_info['ee_collection'])
...
class LandsatImCollection(ImCollection):
    ...
    def _get_image_masks(self, image):
        ...
        fill_mask = qa_pixel.bitwiseAnd(1).eq(1).rename('FILL_MASK')
        ...
        valid_mask = ((cloud_mask.Or(shadow_mask)).Not()).And(fill_mask)

        return masks.update(cloud_mask=cloud_mask, shadow_mask=shadow_mask, fill_mask=fill_mask, valid_mask=valid_mask)


    def get_image(self, image_id, apply_mask=False, add_aux_bands=False, scale_refl=False):
        image = ImCollection.get_image(image_id, apply_mask=apply_mask, add_aux_bands=add_aux_bands)
...
class Sentinel2ImCollection(ImCollection):
    ...
    def _get_image_masks(self, image):
        ...
        return masks.update(cloud_mask=cloud_mask, valid_mask=cloud_mask.Not())


class Sentinel2ClImCollection(ImCollection):
    def __init__(self, collection='sentinel2_toa'):
        ImCollection.__init__(self, collection=collection)
        self.base_collection = self._get_base_collection()
        self._im_transform = ee.Image.toUint16
    ...
    def _get_image_masks(self, image):
        ...
        return masks.update(cloud_mask=cloud_mask, shadow_mask=shadow_mask, valid_mask=valid_mask)
