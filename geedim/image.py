...
# Note:
# - There are a some problems with *int64: While gdal >= 3.5 supports it, rasterio
# casts the nodata value to float64 which cannot represent the int64 range.  Also,
# EE provides an int64 ee.Image (via ee.Image.getDownloadUrl() or
# ee.data.computePixels()) as float64 with nodata advertised as -inf but actually
# zero.  So no geedim *int64 support for now...
# - See also https://issuetracker.google.com/issues/350528377, although EE provides
# uint32 images correctly now.
# - The ordering of the keys above is relevant to the auto dtype and should be:
# unsigned ints smallest - largest, signed ints smallest to largest, float types
# smallest to largest.
...
def _open_raster(ofile: OpenFile, **profile) -> DatasetWriter:
    ...
    if isinstance(ofile.fs, LocalFileSystem):
        ...
        # Otherwise, cache the raster in memory then write the cache to file with
        # fsspec. Rather than pass a file object to rio.open(), this manages
        # MemoryFile and dataset contexts to work around
        # https://github.com/rasterio/rasterio/issues/3064.
...
def _build_overviews(
    ds: DatasetWriter, max_num_levels: int = 8, min_level_pixels: int = 256
):
    """Build internal overviews for an open dataset.  Each overview level is
    downsampled by a factor of 2.  The number of overview levels is determined by
    whichever of the ``max_num_levels`` or ``min_level_pixels`` limits is reached first.
    """
...
def _rio_crs(crs: str | rio.CRS) -> str | rio.CRS:
    """Convert an EE CRS string to a Rasterio compatible CRS string."""
    if crs == 'SR-ORG:6974':
        # This is a workaround for https://issuetracker.google.com/issues/194561313,
        # that replaces the alleged GEE SR-ORG:6974 with actual WKT for SR-ORG:6842
        # taken from https://github.com/OSGeo/spatialreference.org/blob/master
        # /scripts/sr-org.json.
...
def _scale_offset_image(ee_image: ee.Image, stac: dict[str, Any] | None) -> ee.Image:
    ...
    if not stac or (
        set(scale_dict.values()) == {1} and set(offset_dict.values()) == {0}
    ):
        warnings.warn(
            'Cannot scale and offset this image, there is no STAC scale and offset '
            'information.',
...
class ImageAccessor:
    ...
    def _min_projection(self) -> dict[str, Any]:
        ...
        if len(bands) > 0:
            ...
            transforms = [
                (Affine(*bi['crs_transform']) * Affine.translation(*bi['origin']))[:6]
                if 'origin' in bi
    ...
    def date(self) -> datetime | None:
        """Acquisition date & time.  ``None`` if the ``system:time_start`` property
        is not present.
        """
    ...
    def shape(self) -> tuple[int, int] | None:
        """(height, width) dimensions of the minimum scale band in pixels. ``None``
        if the image has no fixed projection.
        """
    ...
    def dtype(self) -> str | None:
        ...
        def get_min_int_dtype(data_types: list[dict]) -> str | None:
            ...
            # find min integer type that can represent the min/max range (relies on
            # ordering of _nodata_vals)
        ...
        dtype = None
        data_types = [
            band_info['data_type'] for band_info in self.info.get('bands', [])
        ]
    ...
    def profile(self) -> dict[str, Any] | None:
        """Export image profile for Rasterio.  ``None`` if the image has no fixed
        projection.
        """
        if not self.shape:
            return None
    ...
    def specBands(self) -> list[str]:
        ...
        band_props = {bp['name']: bp for bp in band_props}
        return [
            bn for bn in self.bandNames if 'center_wavelength' in band_props.get(bn, {})
        ]
    ...
    def _write_metadata(self, ds: DatasetWriter):
        ...
        license_link = [
            lnk.get('href', None) for lnk in links if lnk.get('rel', '') == 'license'
        ]
        if license_link:
        ...
        # TODO: make writing scales/offsets an option and consistent with a similar
        #  xarray option for setting scale/offset/units in the encoding
    ...
    def projection(self, min_scale: bool = True) -> ee.Projection:
        ...
        :param min_scale:
            Whether to return the projection of the minimum (``True``), or maximum
            (``False``) scale band.
        ...
        projs = bands.map(
            lambda band: self._ee_image.select(ee.String(band)).projection()
        )
        scales = projs.map(lambda proj: ee.Projection(proj).nominalScale())
    ...
    def resample(self, method: ResamplingMethod | str) -> ee.Image:
        ...
        See https://developers.google.com/earth-engine/guides/resample for background
        information.

        :param method:
            Resampling method to use.  With the
            :attr:`~geedim.enums.ResamplingMethod.average` method, the image is
            reprojected to the minimum scale projection before resampling.

        :return:
            Resampled image if the source has a fixed projection, otherwise the
            source image.
        """
        ...
        def _resample(ee_image: ee.Image) -> ee.Image:
            ...
            if method is ResamplingMethod.average:
                # set the default projection to the minimum scale projection
                # (required for e.g. S2 images that have non-fixed projection bands)
                ee_image = ee_image.setDefaultProjection(self.projection())
                return ee_image.reduceResolution(reducer='mean', maxPixels=1024)
            else:
                return ee_image.resample(str(method.value))

        return ee.Image(
            ee.Algorithms.If(self.fixed(), _resample(self._ee_image), self._ee_image)
        )
    ...
    def scaleOffset(self) -> ee.Image:
        """
        Apply any STAC scales and offsets to the image (e.g. for converting digital
        numbers to physical units).

        :return:
            Scaled and offset image if STAC scales and offsets are available,
    ...
    def regionCoverage(
        ...
        **kwargs,
        ...
        """
        Find the percentage of a region covered by each band of this image.  The
        image is treated as a mask image e.g. as returned by :meth:`ee.Image.mask`.
        ...
        :param scale:
            ...
            image bands.
        :param kwargs:
            Optional keyword arguments for :meth:`ee.Image.reduceRegion`. Defaults to
            ``bestEffort=True``.
        ...
        # Find the coverage as the sum over the region of the image divided by the
        # sum over the region of a constant (==1) image.  Note that a mean reducer
        # does not find the mean over the region, but the mean over the part of the
        # region covered by the image.
        sums_image = ee.Image([self._ee_image.unmask(), ee.Image(1).rename('ttl')])
        # use crs=proj rather than crs=proj.crs() as some projections have no CRS
        # EPSG string
    ...
    def addMaskBands(self, **kwargs) -> ee.Image:
        ...
        :param bool mask_cirrus:
            Whether to mask cirrus clouds.  Valid for Landsat 8-9 images, and for
            Sentinel-2 images with the :attr:`~geedim.enums.CloudMaskMethod.qa`
            ``mask_method``.  Defaults to ``True``.
        :param bool mask_shadows:
            Whether to mask cloud shadows.  Valid for Landsat images, and for
            Sentinel-2 images with the :attr:`~geedim.enums.CloudMaskMethod.qa` or
            :attr:`~geedim.enums.CloudMaskMethod.cloud_prob` ``mask_method``.
            Defaults to ``True``.
        :param bool mask_saturation:
            Whether to mask saturation.  Valid for Landsat images.  Defaults to
            ``False``.
        :param bool mask_nonphysical:
            Whether to mask reflectance outside the 0-1 reflectance range.  Valid for
            Landsat surface reflectance images, and Sentinel-2 images.  Defaults to
            ``False``.
        ...
        :param float prob:
            ...
            Defaults to ``60``.
        :param float dark:
            NIR threshold [0-1]. NIR values below this threshold are potential cloud
            shadows. Valid for Sentinel-2 images with the
            :attr:`~geedim.enums.CloudMaskMethod.qa` or
            :attr:`~geedim.enums.CloudMaskMethod.cloud_prob` ``mask_method``.
            Defaults to ``0.15``.
        :param int shadow_dist:
            Maximum distance (m) to look for cloud shadows from cloud edges.  Valid
            for Sentinel-2 images with the :attr:`~geedim.enums.CloudMaskMethod.qa`
        ...
        :param float cdi_thresh:
            Cloud Displacement Index threshold. Values below this threshold are
            considered potential clouds. If this parameter is ``None`` (the default),
            the index is not used. Valid for Sentinel-2 images with the
            :attr:`~geedim.enums.CloudMaskMethod.qa` or
        ...
        :param ~geedim.enums.CloudScoreBand cs_band:
            ...
            :attr:`~geedim.enums.CloudMaskMethod.cloud_score` ``mask_method``.
            Defaults to :attr:`~geedim.enums.CloudScoreBand.cs`.
    ...
    def prepareForExport(
        ...
        .. warning::
            Depending on the supplied arguments, the prepared image may be a
            reprojected and clipped version of the source.  This type of image is
            `not recommended <https://developers.google.com/earth-engine/guides
            /best_practices>`__ for use in map display or further computation.

        :param crs:
            CRS of the prepared image as a well-known authority (e.g. EPSG) or WKT
            string. Defaults to the CRS of the minimum scale band.
        :param crs_transform:
            Georeferencing transform of the prepared image, as a sequence of 6
        ...
        :param scale:
            ...
            the image bands.  Ignored if ``crs_transform`` is supplied.
        :param resampling:
        ...
        :param dtype:
            Data type of the prepared image (``uint8``, ``int8``, ``uint16``,
            ``int16``, ``uint32``, ``int32``, ``float32`` or ``float64``).  Defaults
            to the minimum size data type able to represent all image bands.
        :param scale_offset:
            Whether to apply any STAC band scales and offsets to the image (e.g. for
            converting digital numbers to physical units).
        :param bands:
            Bands to include in the prepared image as a list of names / indexes,
            or a regex string.  Defaults to all bands.

        :return:
            Prepared image.
        """
        # Create a new ImageAccessor if bands are supplied.  This is done here so
        # that crs, scale etc parameters used below will have values specific to bands.
        exp_image = ImageAccessor(self._ee_image.select(bands)) if bands else self

        # Prevent exporting images with no fixed projection unless arguments defining
        # the export pixel grid and bounds are supplied (EE allows this in some
        # cases, but uses a 1 degree scale in EPSG:4326 with global bounds, which is
        # an unlikely use case prone to memory limit errors).
        if (
            ...
            raise ValueError(
                'The image does not have a fixed projection, you need to provide '
                "'crs', 'region' & 'scale' / 'shape'; or 'crs', 'crs_transform' & "
                "'shape'."
            )

        if scale and shape:
            # Earth Engine raises an exception in this situation, but its message is
            # obscure
        ...
        # set a default crs argument when crs_transform is provided (crs is required
        # with crs_transform)
        ...
        # convert dtype (required for EE to set nodata correctly on download even if
        # dtype is unchanged)
    ...
    def toGoogleCloud(
        ...
        **kwargs,
        ...
        if type == ExportType.drive:
            # move folders in 'filename' to sub-folders in 'folder' ('filename'
            # should be the filename only)
            ...
            # move sub-folders in 'folder' to parent folders in 'filename' ('bucket'
            # arg should be the bucket name only)
    ...
    def toGeoTIFF(
        ...
        Export projection and bounds are defined by :attr:`crs`, :attr:`transform`
        and :attr:`shape`, and data type by :attr:`dtype`. :meth:`prepareForExport`
        can be called before this method to apply export parameters.
        ...
        if driver is Driver.gtiff:
            profile.update(
                interleave='band', tiled=True, blockxsize=512, blockysize=512
            )
        else:
            # use existing overviews built with DatasetWriter.build_overviews()
            profile.update(
                interleave='band', blocksize=512, overviews='force_use_existing'
            )

        with ExitStack() as exit_stack:
            # Use GDAL_NUM_THREADS='ALL_CPUS' for building overviews once the
            # download is complete.  GDAL_NUM_THREADS is overridden in
            # Tiler.map_tiles() to control download concurrency.
    ...
    def toNumPy(
        ...
        :param masked:
            Return a :class:`~numpy.ndarray` with masked pixels set to the
            :attr:`nodata` value (``False``), or a :class:`~numpy.ma.MaskedArray`
            (``True``).
        :param structured:
            Return a 3D array with (row, column, band) dimensions and a numerical
            ``dtype`` (``False``), or a 2D array with (row, column) dimensions and a
            structured ``dtype`` representing the image bands (``True``).
        :param max_tile_size:
            Maximum tile size (MB).  Should be less than the `Earth Engine size limit
            <https://developers.google.com/earth-engine/apidocs/ee-image
            -getdownloadurl>`__ (32 MB).
        ...
        if structured:
            dtype = np.dtype(
                dict(names=self.bandNames, formats=[self.dtype] * len(self.bandNames))
            )
    ...
    def toXarray(
        ...
        :param masked:
            ...
            (``False``), or to NaN (``True``).  If ``True``, the image :attr:`dtype`
            is integer, and one or more pixels are masked, the returned array is
            converted to a minimal floating point type able to represent :attr:`dtype`.
        :param max_tile_size:
            Maximum tile size (MB).  Should be less than the `Earth Engine size limit
            <https://developers.google.com/earth-engine/apidocs/ee-image
            -getdownloadurl>`__ (32 MB).
        ...
        if not self.transform[1] == self.transform[3] == 0:
            raise ValueError(
                "'The image cannot be exported to Xarray as its 'transform' is not "
                'aligned with its CRS axes.  It should be reprojected first.'
            )
        ...
        return xarray.DataArray(
            data=array, coords=coords, dims=['y', 'x', 'band'], attrs=attrs
        )
