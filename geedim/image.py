...
class ProcImage(Image):
    ...
    def from_id(cls, image_id, mask=False, scale_refl=False):
        ...
        if gd_coll_name != cls._gd_coll_name:
            raise ValueError(
                f"{cls.__name__} only supports images from {info.gd_to_ee[cls._gd_coll_name]}"
            )
...
class Sentinel2Image(ProcImage):
    @staticmethod
    def _im_transform(ee_image):
        return ee.Image.toUint16(ee_image)
    ...
    def _get_image_masks(self, ee_image):
        ...
        masks = ProcImage._get_image_masks(self, ee_image)
...
class Sentinel2ClImage(ProcImage):
    def __init__(self, ee_image, mask=False, scale_refl=False):
        """
        Earth engine image wrapper for cloud/shadow masking and quality scoring
        ...
        self._cloud_prob_thresh = (
            35  # Cloud probability (%); values greater than are considered cloud
        )
        # self._nir_drk_thresh = 0.15# Near-infrared reflectance; values less than are considered potential cloud shadow
        self._cloud_proj_dist = (
            1  # Maximum distance (km) to search for cloud shadows from cloud edges
        )
        self._buffer = 100  # Distance (m) to dilate the edge of cloud-identified objects

        ProcImage.__init__(self, ee_image, mask=mask, scale_refl=scale_refl)
    ...
    def _get_image_masks(self, ee_image):
        ...
        if self.gd_coll_name == "sentinel2_sr":  # use SCL to reduce shadow_mask
            ...
            shadow_mask = proj_cloud_mask.rename(
                "SHADOW_MASK"
            )  # mask all areas that could be cloud shadow
    ...
    def ee_collection(cls):
        ...
        inner_join = ee.ImageCollection(
            ee.Join.inner().apply(s2_sr_toa_col, s2_cloudless_col, filter)
        )
...
class ModisNbarImage(ProcImage):
    @staticmethod
    def _im_transform(ee_image):
        return ee.Image.toUint16(ee_image)
